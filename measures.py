...
def get_seat_wise_analysis(schedule_id=None, hours_before_departure=None, date_of_journey=None):
    ...
    # If hours_before_departure is specified and we have schedule_id, filter by hours before departure
    if df is not None and not df.empty and schedule_id is not None and hours_before_departure is not None:
        # Get the TimeAndDateStamp values that match the selected hours_before_departure
        ...
        if hbd_df is not None and not hbd_df.empty:
            # Filter the seat-wise data to only include rows with matching TimeAndDateStamp
            df = df[df['TimeAndDateStamp'].isin(hbd_df['TimeAndDateStamp'])]
    ...
    # Calculate delta
    df['delta'] = df.apply(lambda row: calculate_price_delta(row['actual_fare'], row['final_price']), axis=1)
